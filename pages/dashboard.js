...
export default function Dashboard() {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(false); // Default to false (secure by default)

  useEffect(() => {
    // Attempt to fetch messages immediately to verify session
    fetchMessages();
    
    const interval = setInterval(() => {
      // Only continue polling if we think we are logged in
      fetchMessages(true);
    }, 5000);
  ...
  const fetchMessages = async (isRefresh = false) => {
    ...
    try {
      ...
      if (data.success) {
        setIsLoggedIn(true); // Session verified
        ...
        // If fetch fails with auth error, redirect
  ...
  if (!isLoggedIn) return null; // Should redirect shortly
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {username === 'admin' && (
                        <Card sx={{ p: 2, bgcolor: '#fff' }}>
                            <Typography variant="h6" gutterBottom color="text.primary">Send Broadcast Notification</Typography>
                            <TextField 
                                label="Title" 
                                fullWidth 
                                margin="normal" 
